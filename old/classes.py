...
class BlackDetector(Detector):
  def __call__(this, theInsertedPointsList, theSequense):
    thisData = theInsertedPointsList[0, 0]
    thisColor = thisData['color']
    if (thisColor[0] <= this.thisValue) or (thisColor[1] <= this.thisValue) or (thisColor[2] <= this.thisValue):
      theInsertedPointsList.append(thisData['coordinate'])
class WaterMarkDetectorSimple(Detector):
  def newDetections(this, element=None):
    this.currentData = {"fixedWatermarkCoefficients": [0, 0, 0], "fixedWatermarkColorOffset": [0, 0, 0], "fixedWatermarkLength": 0, "countOfTheIterationsForDisablingOfFixingNewWatermark": 0}
  def __call__(this, theInsertedPointsList, theSequense):
    parametersOfTheWatermark = theInsertedPointsList.getNextPixelsSimpleWatermarkDataIfNoLess(3, this.thisValue)
    if isDefined(parametersOfTheWatermark):
      this.currentData["fixedWatermarkCoefficients"] = parametersOfTheWatermark.coefficients
      this.currentData["fixedWatermarkColorOffset"] = parametersOfTheWatermark.offset
      this.currentData["fixedWatermarkLength"] = 0
      this.currentData["countOfTheIterationsForDisablingOfFixingNewWatermark"] = 3
      theSequense.fixPossibleWatermarkEdge(theInsertedPointsList[0, 0]["Coordinate"], parametersOfTheWatermark)
      theSequense.append(theInsertedPointsList[0, 0]["Coordinate"])
...
class ImagePart(object):
  def __init__(this, image, offset, theDirection = 0, directionIndex = None, isAhead = None):
    this.image = image
    this.offset = offset
    this.direction = theDirection
    this.directionIndex = directionIndex
    this.isAhead = isAhead
  def getNextPixelsSimpleWatermarkDataIfNoLess(this, maxCount):
    toContinue = True
    thisIndex = 0
    oldColor = thisElementColor = this.getByIndex(thisIndex)["color"]
    newColor = nextElementColor = this.getByIndex(thisIndex + 1)["color"]
    thisCoefficients = detectColorDifferenceFast(thisElementColor, nextElementColor)
    theSumCoefficients = [0, 0, 0]
    isPassed = False
    while((thisIndex < maxCount) or (isNotEmptyColorDifference(thisCoefficients))):
      thisElementColor = nextElementColor
      nextElementColor = this.getByIndex(thisIndex + 1)["color"]
      theSumCoefficients[0] += thisCoefficients[0]
      theSumCoefficients[1] += thisCoefficients[1]
      theSumCoefficients[2] += thisCoefficients[2]
      if isCoefficientsOffsetMore(theSumCoefficients, 20) and (not isPassed):
        isPassed = True
      thisCoefficients = detectColorDifferenceFast(thisElementColor, nextElementColor)
    #return {'coefficients': , 'offset': thisCoefficients} if isPassed else None
  def getByIndex(this, index):
    index = int(index)
    if this.isAhead == False:
      index *= -1
...
class Image(MyArrayDictionary):
  def processPairs(this, handler, intervals = None):
    ...
    for thePoint[0] in theXRange:
      ...
      for thePoint[1] in theYRange:
        ...
        if isDefined(prev):
          handler(ImagePart(this, thePoint, theDirection = coef), currentData)
  ...
  def getColorFast(this, pixel):
    try:
      ...
      result = None
    return result
  def setColorFast(this, pixel, color):
    try:
      result = this.inColor[pixel[1]][pixel[0]]
      result[0] = color[0]
      result[1] = color[1]
...
class CoordinatesSet(Coordinates, MyArraySet):
  def __init__(this, values = None):
    ...
    #  print('El: {}'.format(el))
    return None
def getHashOfCoordinate(coordinate):
  return coordinate[0] * 10000 + coordinate[1]
class CoordinatesMap(object):
  def __init__(this, size):
    this.data = {"size": size, "elements": dict(), "indexes": {"by-elements": dict(), "values": []}}
    thisValues = [float(this.size[0]), float(this.size[1])]
    theIndexes = this.data['indexes']['values']
    for index in range(CoordinatesMap.indexesCount):
      theIndexes.append([])     
    sizeOfTheSide = 1
    partsCount = 1
    beforeIndex = None
    beforeIndexes = None
    theCurrentParent = None
    for index in reversed(range(CoordinatesMap.indexesCount)):
      thisIndexes = theIndexes[index]
      sideBeforeIndex = None
      startOffset = 0
      stepsForChange = [sizeOfTheSide, 1]
      theCountToChange = list(map(stepsForChange, lambda value: value * 2))
      theCurrentOffsetToTheCountOfChange = [0, 0]
      for thisIndex in range(partsCount):
        for indexOfTheChange in range(2):          
          if theCurrentOffsetToTheCountOfChange[indexOfTheChange] == theCountToChange[indexOfTheChange]:
            startOffset += stepsForChange[indexOfTheChange]
            if isDefined(theCurrentParent):
              theCurrentParent = beforeIndexes[startOffset]
          theCurrentOffsetToTheCountOfChange = 0        
        theCurrentOffsetToTheCountOfChange[thisIndex] += 1
      thisElement = {"next": [], "has": False, "parent": theCurrentParent}
      if isDefined(beforeIndexes):
        beforeIndexes[startOffset]['next'].append(thisElement)
      thisIndexes.append(thisElement)
      beforeIndex = index
      beforeIndexes = theIndexes[beforeIndex]
      theCurrentParent = beforeIndexes[startOffset]
      sizeOfTheSide *= 2
      partsCount *= 4      
    this.sizeOfTheSide = sizeOfTheSide
  def processToFirstPointThatHas(this, coordinate, setHavingElement = False):
    element = this.getIndexOfTheCoordinate(coordinate)
    while isDefined(element) and (not (element['has'])):
      if setHavingElement:
        element['has'] = True
      element = element['parent']
    return element
  def getIndexInIndexesOfTheCoordinate(this, coordinate):
    return this.sizeOfTheSide * (int(coordinate[1]) / this.sizeOfTheSide) + int(coordinate[0]) / this.sizeOfTheSide
  def getIndexOfTheCoordinate(this, coordinate):
    return this.data['indexes']['values'][this.getIndexInIndexesOfTheCoordinate(coordinate)]
  def getNearest(this, coordinate):
    havingElement = this.processToFirstPointThatHas(coordinate)
  def set(this, coordinate, value):
    pass
    """theHash = CoordinatesMap.getHash(coordinate)
    isNew = (theHash not in this.data['indexes']['by-elements'])
    this.data['elements'][theHash] = value
    if isNew:
      this.processToFirstPointThatHas(coordinate, True)                 
CoordinatesMap.indexesCount = 21
CoordinatesMap.getHash = getHashOfCoordinate
"""
class CoordinatesSequence(Coordinates, MyArrayList):
  def fixPossibleWatermarkEdge(this, coordinate, parametersOfTheWatermark):
    pass
  def __init__(this, values = None):
